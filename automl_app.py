...
if st.session_state.mode is None:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("### 🆕 새 예측모델 만들기")
        st.write("CSV/Excel 데이터로 여러 모델을 자동 학습·비교하고, 최적 모델을 저장할 수 있습니다.")
        if st.button("새 모델 만들기", type="primary", width="stretch"):
            st.session_state.mode = "train"
            st.rerun()
    with c2:
        st.markdown("### 📂 기존 결과 불러와서 예측하기")
    ...
    st.markdown(
        ...
        'border-top:1px solid #c5dda0; padding:8px 20px; text-align:right; '
        'font-size:0.8rem; line-height:1.55; z-index:1000;">'
